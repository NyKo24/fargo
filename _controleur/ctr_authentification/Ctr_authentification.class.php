...
class Ctr_authentification extends Controleur {
    ...
    function connexion() {
        if (isset($_POST["login"])) 
        {
            extract($_POST);
            $mdp=cryptage($mdp);
            $query="select * from utilisateur where (uti_email='$login') and uti_mdp='$mdp' ";
            $result=Table::$con->query(Table::$con->real_escape_string($query));
            if (isset($_POST["souvenir"]))
            {
            	setcookie("login",$uti_login,time()+60*60*24*30,"/");
            	setcookie("mdp",$uti_mdp,time()+60*60*24*30,"/");
            }
            if ($row=$result->fetch_assoc()) 
            {
                ...
                $_SESSION["uti_profil"]=$row["uti_type"];
                $_SESSION["uti_email"]=$row["uti_email"];
                
				header("location:" . BASE_URL . "_default/index");
            } 
            else
            {
                self::flashMessage()->error('Identifiants incorrects');
                header("location:" . BASE_URL . "authentification/connexion");
            } 
        } 
        else 
        {
            require BASE_REP . "_controleur/ctr_authentification/vue_authentification_connexion.php";
        }
    }
     ...
     */
    function mdpoublie(){
    	require BASE_REP . "_controleur/ctr_authentification/vue_authentification_mdpoublie.php";
    }
    ...
    function resetmdp(){
    	...
    	if ($row=$result->fetch_assoc()){
    		...
    		if(!is_null($row["uti_email"]) or !empty($row["uti_email"])){
    			envoyerUnMail(array($row["uti_email_perso"]), "Mot de passe oublié", $chaine,"system@yourdomain.tld");
    		}
